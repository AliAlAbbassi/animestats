<template>
  <c-box v-if="show" border-width="0px" rounded="lg" overflow="hidden" d="flex">
    <c-image
      class="animecover"
      :src="show.animeImagePath"
      :alt="show.animeTitle"
    />
    <c-box class="scndsection">
      <c-box d="flex" align-items="baseline">
        ...
        >
          {{ show.animeNumEpisodes }} Episodes &bull;
          {{ show.numWatchedEpisodes }} Watched
        </c-box>
      </c-box>
      <c-box
        font-weight="semibold"
      ...
      <c-box>
        {{ show.animeAiringStatus }}
        <c-box as="span" color="gray.600" fontSize="sm">
          / {{ show.animeNumEpisodes }}
        </c-box>
      </c-box>
      <c-box d="flex" align-items="center">
        <c-icon
          ...
          name="star"
          :key="i"
          :color="i < show.score ? 'green.500' : 'gray.300'"
        />
      </c-box>
  ...
  <p v-else class="text">No Anime added</p>
</template>
